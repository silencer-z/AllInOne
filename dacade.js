...
game.import("extension", function(lib,game,ui,get,ai,_status){return { name:"十周年UI",
        ...
        lib.skill._longLevel = {
            ...
            content: function () {
                ...
                if (lib.config.extension_十周年UI_longLevel == 'seven') {
                    ...
                    //	var rarity
                    //	if (game.getRarity(player.name) == 'junk') rarity = 'yin';
                    //	if (game.getRarity(player.name) == 'common') rarity = 'gold';
                    //	if (game.getRarity(player.name) == 'rare') rarity = 'yu';
                    //	if (game.getRarity(player.name) == 'epic') rarity = 'bing';
                    //	if (game.getRarity(player.name) == 'legend') rarity = 'yan';
                }
            ...
            initOverride:function(){
                ...
                var EventContent = (function(EventContent){
                    ...
                    EventContent.equip = function () {
                        ...
                        /*-----------------分割线-----------------*/
                        // 新样式装备栏，Engj.K大佬提供代码
                        if (lib.config.extension_十周年UI_equipStyle == 'off') {
                                ...
                                for (i = 0; i < e.length; i++) {
                                    if (i == 0) {
                                        if (colour == 'heart' || colour == 'diamond') {
                                            e[i].style.color = "#EF1806"; // 方块/红桃花色颜色显示
                                        } else {
                                            e[i].style.color = "#8DBEDE"; // 梅花/黑桃花色颜色显示
                                        }
                                        e[i].style.marginLeft = "3px"; //装备花色字体整体右移
                                    } else {
                                        e[i].style.color = "#E9E8E3"; // 装备字体颜色显示
                                        e[i].style.marginLeft = "6px"; // 装备字体单独右移
                                    }
                                    e[i].style.textShadow = "1px 0 0 black, 0 1px 0 black, -1px 0 0 black, 0 -1px 0 black"; // 装备字体描边显示
                ...
                // 手杀UI计时器更改
                /*
		// lib.config.extension.contains ('扩展名') 检测已安装扩展
		// lib.config.extension_扩展名__enable 检测已打开扩展
                                ...
                                if (lib.config.cardtempname != 'off') {
                                    ...
                                    if ((cards[i].name != cardname) || (cards[i].nature != cardnature) || (cards[i].suit != cardsuit) || (cards[i].number != cardnumber)){
									 ...
									 if (cardnature&&!['huogong'].contains(cardname)) {
										...
										cards[i]._tempName.dataset.name = cardname;
                        ...
                        innerHTML:{
                            ...
                            set:function(value){
                                ...
                                if (decadeUI.config.campIdentityImageMode) {
                                    ...
                                    // 不同样式身份标记
                                    if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
                                        image.src = extensionPath + 'image/decoration/identity_' + filename + '.png'; } else {
                                ...
                                if (value){
                                    if (decadeUI.config.campIdentityImageMode){
                                        ...
                                        if (decadeUI.config.newDecadeStyle == 'off') {
                                            var url = extensionPath + 'image/decorations/name2_' + value + '.png'; } else {
    ...
    precontent:function(){
        ...
        window.decadeModule = (function(decadeModule){
            ...
            decadeModule.init = function () {
                /*-----------------分割线-----------------*/
                /*本体文件*/
                //this.css(decadeUIPath + 'splash.css');//弃用
                /*-----------------分割线-----------------*/
                ...
                // 不同样式文件调用
                if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
                    this.css(decadeUIPath + (lib.config.touchscreen?'player1.css':'player1_PC.css')); }
                if (lib.config.extension_十周年UI_newDecadeStyle == 'off') {
                    this.css(decadeUIPath + (lib.config.touchscreen?'player2.css':'player2_PC.css')); }
                if (lib.config.extension_十周年UI_newDecadeStyle == 'othersOn') {
                    this.css(decadeUIPath + (lib.config.touchscreen?'player3.css':'player3_PC.css')); }

                /*-----------------分割线-----------------*/
                // 不同样式装备栏调用
                if (lib.config.extension_十周年UI_equipStyle == 'on') {
                    this.css(decadeUIPath + 'equip.css'); }
                if (lib.config.extension_十周年UI_equipStyle == 'off') {
                    this.css(decadeUIPath + 'equip_new.css'); }
    ...
    config:{
        ...
        equipStyle: {
            name: '<b><font color=\"#00FFFF\">装备栏样式(需重启)',
            intro: '<b><font color=\"#00FFFF\">可以根据个人喜欢选择不同的装备栏风格，包括有经典样式与新样式，亦可选择随机调用',
            init: 'on',
            item: {
                on: '经典配色',
                off: '新版配色',
            ...
            onclick:function(value){
                game.saveConfig('extension_十周年UI_equipStyle', value);
                if (window.decadeUI) decadeUI.config.equipStyle = value;
            },
        },
        loadingStyle: {
